...
export default class Search{
    ...
    sendRequest(){
      axios.post('/search', {_csrf: this._csrf, searchTerm: this.inputField.value}).then(response => {
        ...
        alert("Send request just failed.")
      })
    }
    ...
    renderResultsHTML(posts){
      if(posts.length){
        ...
        this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we coould not found any result for that search.</p>`
      }
      this.hideLoaderIcon()
